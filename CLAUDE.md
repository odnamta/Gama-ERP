# GIS-ERP (PT. Gama Intisamudera ERP)

> **Sync Note**: Keep this file in sync with `.kiro/steering/Project Context.md`.
...
```typescript
import { formatDate, formatDateTime, formatCurrency, formatCurrencyShort } from '@/lib/utils/format'
```
**DO NOT** use `toLocaleDateString()`, `new Intl.DateTimeFormat()`, or deprecated `lib/pjo-utils.ts`.
...
// Tables not in generated types (Co-Builder competition):
...
## MCP Servers
- **Supabase** - Database management (`.claude/mcp.json`)
