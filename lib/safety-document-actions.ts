...
export async function createSafetyDocument(
  ...
  try {
    ...
    // NOTE: created_by FK references auth.users(id), so use user.id (auth UUID), NOT userProfile.id
    ...
    if (insertError) {
      console.error('[createSafetyDocument] Supabase error:', insertError.code, insertError.message, insertError.details);
...
export async function createNewVersion(
  ...
  try {
    ...
    // Create new version
    // NOTE: created_by FK references auth.users(id), so use user.id (auth UUID)
